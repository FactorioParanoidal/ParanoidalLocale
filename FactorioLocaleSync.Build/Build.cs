...
partial class Build : NukeBuild
{
    ...
    Target CloneParanoidalRepository => _ => _
        .Executes(() =>
        {
            ...
            ParanoidalDirectory.CreateOrCleanDirectory();
            GitTasks.Git("clone https://github.com/FactorioParanoidal/ParanoidalModpack .", ParanoidalDirectory,
                logger: (type, s) => Log.Debug(s));
        });
    ...
    Target ExtractLocalizationToJsons => _ => _
        ...
        .Executes(() =>
        {
            ...
            var modLocalesToProcess = ModLocalizationUtils
                .ProcessModsToGetLocalizable(modInfos, TargetLocale, Log.Logger).ToList();
            var modsToProcess = modLocalesToProcess.GroupBy(locale => locale.Mod).Select(locales => locales.Key)
                .ToList();
            Log.Information("Found {FilesCount} files from {ModsCount} mods to process",
    ...
    Target UpdateAndPackMod => _ => _
        .Executes(() =>
        {
            ...
            {
                var newVersion = new Version(infoJson.Version.Major, infoJson.Version.Minor,
                    infoJson.Version.Build + 1);
                Log.Information("Locales changed, bumping version from {OldVersion} to {NewVersion}", infoJson.Version,
                    newVersion);
            ...
            if (infoJson.Dependencies!.Count != dependencies.Count)
            {
                ...
                Log.Information("Dependencies count changed, bumping version from {OldVersion} to {NewVersion}",
                    infoJson.Version, newVersion);
                infoJson.Version = newVersion;
            }
